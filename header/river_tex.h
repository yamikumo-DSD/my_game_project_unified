...
{
	class RiverTexture : public DynamicObject
	{
	public:
		struct CameraPos
		{
...
//       y coord                                 
	...
	public:
		...
		RiverTexture(RealType1&& _constant_x_coord, RealType2&& _initial_z_coord) noexcept
			: z(std::forward<RealType2>(_initial_z_coord)),
			x(std::forward<RealType1>(_constant_x_coord)) {}
		RiverTexture(const RiverTexture& _tex) noexcept = default;
		RiverTexture& operator=(const RiverTexture& _tex) noexcept { return *this = _tex; }
		RiverTexture(RiverTexture&& _tex) noexcept = default;
		RiverTexture& operator=(RiverTexture&& _tex) noexcept = default;
		...
		virtual void draw(void) const override final
		{
			...
			{
				z < z_horizon() ?
				RiverTexture::x_intercept(camera_pos.x, camera_pos.z, x, z)
				:
				RiverTexture::x_intercept(camera_pos.x, camera_pos.z, x, z_horizon())
			};
			const auto x2{ z - TEX_HIGHT > 0 ? RiverTexture::x_intercept(camera_pos.x, camera_pos.z, x, z - TEX_HIGHT) : x };
			const auto x3
			{
				z < z_horizon() ?
				RiverTexture::x_intercept(camera_pos.x, camera_pos.z, x - TEX_WIDTH, z)
				:
				RiverTexture::x_intercept(camera_pos.x, camera_pos.z, x - TEX_WIDTH, z_horizon())
			};
			const auto x4{ z - TEX_HIGHT > 0 ? RiverTexture::x_intercept(camera_pos.x, camera_pos.z, x - TEX_WIDTH, z - TEX_HIGHT) : x - TEX_WIDTH };
			const auto y1
			{
				z < z_horizon() ?
		...
		{
			ImagePool::add("../../data/img/river.png");
			img = ImagePool::get("../../data/img/river.png");
		}
		virtual ~RiverTexture(void) = default;
	};
}
