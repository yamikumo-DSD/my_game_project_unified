...
void ASTAnalyzer::analyze_function(const AST::Function& _function)
{
	...
	for (const auto& mnemonic : _function.definition)
	{
		if (mnemonic.tag)
		{
			if (i <= std::pow(2, 16) - 1)
			{
				...
				taged_mnemonic_list_for_goto.emplace_back(std::move(temp));
			}
			else
			{
				...
				taged_mnemonic_list_for_goto_w.emplace_back(std::move(temp));
			}
		}
		if      (mnemonic.instruction.name == "ldc"                           ) { i += 2; }
		else if (mnemonic.instruction.name == "ldc_w"                         ) { i += 3; }
		...
		else if (mnemonic.instruction.name == "summon_mob_with_variadic_args" ) { i += 1; }
		else if (mnemonic.instruction.name == "user_extention_10"             ) { i += 1; }
		else if (mnemonic.instruction.name == "goto_next_stage"               ) { i += 1; }
		else if (mnemonic.instruction.name == "user_extention_11"             ) { i += 1; }
		else if (mnemonic.instruction.name == "goto_stage"                    ) { i += 2; }
		else if (mnemonic.instruction.name == "user_extention_12"             ) { i += 2; }
	...
	constant_pool.emplace_back(std::move(new_func_ref));
...
#define IF_NO_ARG(INSTRUCTION) \
	OPERAND_NUM_CHECK(0); \
	code_list[current_function_name].bytecode.emplace_back(RepeatableVM::Instruction::INSTRUCTION);
#endif
...
#define ANALYZE_JUMPER(INSTRUCTION) \
		...
		code_list[current_function_name].bytecode.emplace_back(std::get<2>(*result));\
		code_list[current_function_name].bytecode.emplace_back(std::get<3>(*result));
#endif
...
#define LOAD_AND_STORE_NOINDEX(INSTRUCTION)\
		...
		{\
			code_list[current_function_name].bytecode.emplace_back(RepeatableVM::Instruction::INSTRUCTION);\
			code_list[current_function_name].bytecode.emplace_back(static_cast<Byte>(index));\
		}\
		else \
		{\
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	{
		OPERAND_NUM_CHECK(1);
		code_list[current_function_name].bytecode.emplace_back(RepeatableVM::Instruction::LDC);
		code_list[current_function_name].bytecode.emplace_back(static_cast<Byte>(constant_index));
		if (_mnemonic.operand_list[0].type() == typeid(jint))
		{
			constant_pool.emplace_back(boost::get<jint>(_mnemonic.operand_list[0]));
		}
		else if (_mnemonic.operand_list[0].type() == typeid(jfloat))
		{
			constant_pool.emplace_back(boost::get<jfloat>(_mnemonic.operand_list[0]));
		}
		++constant_index;
	}
	else if (_mnemonic.instruction.name == "ldc_w")
	{
		OPERAND_NUM_CHECK(1);
		code_list[current_function_name].bytecode.emplace_back(RepeatableVM::Instruction::LDC_W);
		BitCalculation::T2Byte<Short> temp; temp.t = constant_index;
		code_list[current_function_name].bytecode.emplace_back(static_cast<Byte>(temp.field._1)); //indexbyte1
		code_list[current_function_name].bytecode.emplace_back(static_cast<Byte>(temp.field._0)); //indexbyte2
		if (_mnemonic.operand_list[0].type() == typeid(jint))
		{
			constant_pool.emplace_back(boost::get<jint>(_mnemonic.operand_list[0]));
		}
	...
	{
		OPERAND_NUM_CHECK(1);
		code_list[current_function_name].bytecode.emplace_back(RepeatableVM::Instruction::INVOKE_STATIC);
	...
	else if (_mnemonic.instruction.name == "bipush")
	{
		...
		{
			code_list[current_function_name].bytecode.emplace_back(RepeatableVM::Instruction::BIPUSH);
			code_list[current_function_name].bytecode.emplace_back(static_cast<Byte>(value));
		}
		else
		{
	...
	else if (_mnemonic.instruction.name == "sipush")
	{
		...
		if (value <= std::pow(2, 16) - 1)
		{
			...
			code_list[current_function_name].bytecode.emplace_back(RepeatableVM::Instruction::SIPUSH);
			code_list[current_function_name].bytecode.push_back(temp.field._1);
			code_list[current_function_name].bytecode.push_back(temp.field._0);
		}
		else
	...
	else if (_mnemonic.instruction.name == "iinc")
	{
		...
		if (index <= std::pow(2, 8) - 1)
		{
			...
			{
				code_list[current_function_name].bytecode.emplace_back(RepeatableVM::Instruction::IINC);
				code_list[current_function_name].bytecode.emplace_back(static_cast<Byte>(index));
				code_list[current_function_name].bytecode.emplace_back(BitCalculation::bit_cast<Byte>(static_cast<jchar>(const_)));
			}
			else
			{
	...
	else if (_mnemonic.instruction.name == "user_extention_2" || _mnemonic.instruction.name == "math_func_uni") 
	{
		...
		{
				code_list[current_function_name].bytecode.emplace_back(RepeatableVM::Instruction::USER_EXTENTION_2);
				code_list[current_function_name].bytecode.emplace_back(static_cast<Byte>(function_selected));
		}
		else
		{
	...
	else if (_mnemonic.instruction.name == "user_extention_6" || _mnemonic.instruction.name == "math_func_bi") 
	{
		...
		if (function_selected >= 0 && function_selected <= std::pow(2, 8) - 1)
		{
				code_list[current_function_name].bytecode.emplace_back(RepeatableVM::Instruction::USER_EXTENTION_6);
				code_list[current_function_name].bytecode.emplace_back(static_cast<Byte>(function_selected));
		}
		else
		{
	...
	else if (_mnemonic.instruction.name == "summon_mob_with_variadic_args" ) { IF_NO_ARG(USER_EXTENTION_10)    }
	else if (_mnemonic.instruction.name == "user_extention_10"             ) { IF_NO_ARG(USER_EXTENTION_10)    }
	else if (_mnemonic.instruction.name == "goto_next_stage"               ) { IF_NO_ARG(USER_EXTENTION_11)    }
	else if (_mnemonic.instruction.name == "user_extention_11"             ) { IF_NO_ARG(USER_EXTENTION_11)    }
	else if (_mnemonic.instruction.name == "goto_stage" || _mnemonic.instruction.name == "user_extention_12") 
	{
		OPERAND_NUM_CHECK(2);  
		const auto stage_num{boost::get<jint>(_mnemonic.operand_list[0])};
		if (stage_num >= 0 && stage_num <= std::numeric_limits<Byte>::max())
		{
			code_list[current_function_name].bytecode.emplace_back(RepeatableVM::Instruction::USER_EXTENTION_12);
			code_list[current_function_name].bytecode.emplace_back(static_cast<Byte>(stage_num));
		}
		else
		{
			throw std::runtime_error("The stage number must be in 0-255.");
		}
	}
